...
CREATE INDEX IF NOT EXISTS idx_users_auth_provider_identity ON users(auth_provider, auth_provider_user_id);
...
CREATE INDEX IF NOT EXISTS idx_onboarding_user_status ON onboarding_states(user_id, status);
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'fk_auth_identities_user') THEN
        ALTER TABLE auth_identities ADD CONSTRAINT fk_auth_identities_user FOREIGN KEY (user_id) REFERENCES users(user_id) ON DELETE CASCADE NOT VALID;
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'fk_tutor_profiles_user') THEN
        ALTER TABLE tutor_profiles ADD CONSTRAINT fk_tutor_profiles_user FOREIGN KEY (user_id) REFERENCES users(user_id) ON DELETE CASCADE NOT VALID;
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'fk_onboarding_states_user') THEN
        ALTER TABLE onboarding_states ADD CONSTRAINT fk_onboarding_states_user FOREIGN KEY (user_id) REFERENCES users(user_id) ON DELETE CASCADE NOT VALID;
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'ck_users_account_status') THEN
        ALTER TABLE users ADD CONSTRAINT ck_users_account_status CHECK (status IN ('pending_profile','pending_approval','active','suspended','inactive')) NOT VALID;
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'ck_teacher_approval_status') THEN
        ALTER TABLE teacher_profiles ADD CONSTRAINT ck_teacher_approval_status CHECK (approval_status IN ('incomplete','pending','approved','rejected','suspended')) NOT VALID;
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'ck_onboarding_status') THEN
        ALTER TABLE onboarding_states ADD CONSTRAINT ck_onboarding_status CHECK (status IN ('not_started','in_progress','completed','blocked','requires_review')) NOT VALID;
    END IF;
END $$;
